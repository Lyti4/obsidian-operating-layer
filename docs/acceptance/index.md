...
| Manifest-candidate selector v1 | Hermes-accepted as repo-only selector smoke over existing JSON evidence; selects at most five review candidates and keeps approval/apply authority empty; independent read-only review remains the next acceptance hardening gate | `docs/spec-kit/49-manifest-candidate-selector.md`, `src/obslayer/manifest_candidate_selector.py`, `tools/obsidian_manifest_candidate_selector.py`, `tests/test_manifest_candidate_selector.py`, `out/reports/manifest-candidate-selector/grouped-next5-smoke/HERMES_ACCEPTANCE.md` |
