...
**Статус:** `canonical verify-before-use`.
...
## Last verified Nanobot scheduled loop
...
Verify with `hermes cron list --all` before relying on this table. Role authority
is in `docs/agents/NANOBOT.md`; a standing observer contract does not resume jobs.
This repo-only instruction slice does not change runtime.
